...
<div class="container">
  ...
  <form method="POST" action="proc_exc_musica.php">
  <label>Músicas:</label><br>

<select name="musica" required>
<?php include("conexao.php");
try{
  $musicas= $connect->prepare("SELECT id_musicas,nome_musica,artista,album FROM musicas");
  $musicas->execute();

  foreach ($musicas as $row){
  echo 
  "<option value=".$row['id_musicas'].">".$row['nome_musica']."</option>".
  "<option disabled> Artista: ".$row['artista']."</option>".
  "<option disabled> Album: ".$row['album']."</option>";
}}
